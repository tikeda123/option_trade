...
from aiml.bayes_prediction_model import prediction_option_bayes_model
...
class SimulationStrategy(TradingStrategy):
        ...
        def Idle_event_execute(self, context: SimulationStrategyContext)->None:
                ...
                ema_price = context.dm.get_ema_price()


                if pred == 1 and ema_price > close_price:
                        return

                if pred == 0 and ema_price < close_price:
                        return

                entry_price, exit_price = prediction_option_model(pred,current_date,close_price)
                context.dm.set_entry_price_garch(entry_price)
                context.dm.set_exit_price_garch(exit_price)

                if flag ==False or entry_price is None or exit_price is None:
                        return
