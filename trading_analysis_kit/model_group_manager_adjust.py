...
class ModelGroupManager:
        ...
        def trend_prediction(self, context) -> tuple:
                ...
                from aiml.timeframe_converter import resample_df
                ...
                if self.interval_gp != self.interval:
                        interval_mapping = {
                            ...
                            240: '4H',
                            720: '12H',
                            1440: '24H',  # Future extension
                            2880: '48H'   # Future extension
                        }

                        if self.interval_gp not in interval_mapping:
                            raise ValueError(f"Invalid interval_gp value: {self.interval_gp}. Must be one of {list(interval_mapping.keys())}")

                        interval_aligned = interval_mapping[self.interval_gp]

                        hours = int(self.interval_gp/60)

                        series_period = TIME_SERIES_PERIOD * hours * 3
                        df = self.data_loader.load_data_from_point_date(date, series_period,  MARKET_DATA, self.symbol,  self.interval)
                        df = resample_df(df, to_timeframe=interval_aligned, base_time=date.strftime('%H:%M:%S'))

                        tech_db = TechnicalAnalyzer()
                        df = tech_db.analyze(df)
                        df = df.tail(TIME_SERIES_PERIOD)

                else:
                        interval_aligned = self.interval
                        df = self.data_loader.load_data_from_point_date(date,TIME_SERIES_PERIOD,MARKET_DATA_TECH,self.symbol, interval_aligned)
                ...
                combined_pred = np.sum(
                        [self.model_weights[i] * rolling_pred[i] for i in top_model_indices]
                )
