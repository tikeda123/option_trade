...
from common.utils import  get_higher_timeframe_info


class SimulationStepwiseProfits(TradingStrategy):
        """
        ...
        def should_exit_position(self, context:SimulationStrategyContext, index: int) -> bool:
                ...
                if context.dm .get_entry_counter() > 10:
                        exit_price = context.dm.get_close_price()
                        self._handle_position_exit(context, exit_price, losscut=False)
                        return True

                return False


        def should_exit_preparetaion(self, context, index: int) -> bool:
                """
                Determines whether to exit the current position.
