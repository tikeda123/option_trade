...
class TransformerPredictionRollingModel(BaseModel, PredictionModel):
        ...
        def _prepare_sequences(self, data):
                ...
                # Convert datetime strings to unix timestamps only if 'date' is in feature_columns
                ...
                #  ❶ スケーラを一度だけ fit させる
                # =================================
                # fit() する前に、実際に scaler にかけたい列だけを抽出
                # （ここでは "self.feature_columns" のみを使用していると想定）
                # 全サンプルに対して一括で fit する
        ...
        def create_cnn_transformer_model(
                ...
                This model uses convolutional layers to extract features from the time series data,
                followed by Transformer blocks to capture long-range dependencies.

                Args:
                        input_shape (tuple): Shape of input data.
                        num_heads (int, optional): Number of attention heads in Transformer. Defaults to 16.
                        dff (int, optional): Dimensionality of the feedforward network in Transformer. Defaults to 256.
                        rate (float, optional): Dropout rate. Defaults to 0.1.
                        l2_reg (float, optional): L2 regularization coefficient. Defaults to 0.01.
                        num_transformer_blocks (int, optional): Number of Transformer blocks. Defaults to 4.
                        ...
                        pool_size (int, optional): Pool size for max pooling layers. Defaults to 2.
...
def main():
    ...
    config = get_config_model("MODEL_MIDDLE_TERM", model_id)  # Get the model configuration

    model = TransformerPredictionRollingModel(id=model_id,config=config,data_loader=data_loader,logger=logger,use_gpu=True)

    #model.set_parameters(param_epochs=25, n_splits=2)
    model.load_model()
