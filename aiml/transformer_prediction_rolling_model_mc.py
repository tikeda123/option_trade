...
class TransformerPredictionRollingModel(BaseModel, PredictionModel):
    ...
    def train_with_cross_validation(
        ...
        return scores
    # -------------------------------------------------------------------------
    # ここから下が MC Dropout による不確実性推定メソッドの追加部分
    # -------------------------------------------------------------------------
    def predict_mc_dropout(
        ...
        """
        Makes predictions with MC Dropout enabled (training=True at prediction).

        Args:
            data (np.ndarray): Data to predict for (形状は (サンプル数, シーケンス長, 特徴量数)).
            n_iter (int): Number of forward passes to perform.
            use_gpu (bool, optional): Whether to use GPU for this specific prediction.
                                      If None, uses the model's default setting.

        Returns:
            Tuple[np.ndarray, np.ndarray]:
                - mean_pred (予測平均, shape=(サンプル数, 1))
                - std_pred  (予測標準偏差, shape=(サンプル数, 1))
        """
        ...
        for _ in range(n_iter):
            # training=True を指定し、推論時にも Dropout を有効化
            preds = self.model(data, training=True)
            predictions_list.append(preds)

        # shape: (n_iter, サンプル数, 1)
        stacked_preds = tf.stack(predictions_list, axis=0)
        # 予測平均
    ...
    def evaluate_with_uncertainty(
        ...
        """
        MC Dropout による標準偏差を利用して、自信のあるサンプルだけ評価する。

        Args:
            x_test (np.ndarray): テストデータ (形状は (サンプル数, シーケンス長, 特徴量数))
            y_test (np.ndarray): テストデータの正解ラベル
            std_threshold (float): この値より標準偏差が小さいサンプルのみ抽出して評価
            n_iter (int): MC Dropout 推論の繰り返し回数
            use_gpu (bool, optional): GPU を使うかどうか
        """
        # 1. MC Dropout で全サンプルの mean / std を取得
        mean_pred, std_pred = self.predict_mc_dropout(
            x_test, n_iter=n_iter, use_gpu=use_gpu
        )

        # 2. 二値化する: mean_pred > 0.5 をクラス1、それ以外をクラス0
        #    shape: (サンプル数, 1) を flatten()
        predicted_labels = (mean_pred > 0.5).astype(int).flatten()

        # 3. 標準偏差が閾値未満のサンプルだけ取り出す
        std_values = std_pred.flatten()
        mask_confident = std_values < std_threshold

        x_confident = x_test[mask_confident]
        y_confident = y_test[mask_confident]
        pred_confident = predicted_labels[mask_confident]

        # 4. 各種サイズ・割合を出力
        n_total = len(x_test)
        n_confident = len(x_confident)
        ratio_confident = 100.0 * n_confident / n_total
        print("----------------------------------------------------")
        print(f"Total test samples: {n_total}")
        print(f"Confident samples (std < {std_threshold}): {n_confident} "
              f"({ratio_confident:.2f}%)")

        # 5. 評価結果を算出
        if n_confident == 0:
            print("[Warning] No confident samples found. (std_threshold is too strict?)")
            return

        acc_conf, report_conf, confmat_conf = self._evaluate_predictions(
            y_confident, pred_confident
        )

        # 6. 結果を表示
        print(f"[Confident subset evaluation | std < {std_threshold}]")
    ...
    def _evaluate_predictions(
        ...
        """
        内部ユーティリティ:
        与えられた y_true と y_pred から Accuracy, ClassificationReport, ConfusionMatrix を出力する。
        """
...
def main():
    ...
    print(conf_matrix)
    # -------------------------
    # MC Dropout を使ったフィルタリング評価
    # -------------------------
    model.evaluate_with_uncertainty(
        x_test, y_test,
        std_threshold=0.05,  # たとえば標準偏差0.1より小さいものだけ
        n_iter=100,
        use_gpu=True
    )

    # 例: もっと厳しく自信を要求するなら0.05にするなど
    # model.evaluate_with_uncertainty(x_test, y_test, std_threshold=0.05)
