...
def quantile_loss_factory(quantiles: list):
    """
    複数クォンタイルに対するピンボール損失をまとめて計算するLossを返すFactory関数。
    quantiles: [0.1, 0.5, 0.9] のように複数指定
    """
    def quantile_loss(y_true, y_pred):
        """
        y_true.shape: (batch_size, 1)
        y_pred.shape: (batch_size, len(quantiles))
        """
        # y_trueを各クォンタイル分に拡張
        #   expand_dims -> (batch_size, 1, 1)
        #   then tf.repeat -> (batch_size, len(quantiles), 1)
        #   最終的に shape: (batch_size, len(quantiles))
        y_true_tiled = tf.repeat(y_true, repeats=len(quantiles), axis=1)
        ...
        for i, q in enumerate(quantiles):
            errors_q = e[:, i]
            # ピンボール損失： max(q * error, (q-1) * error)
            loss_q = tf.maximum(q * errors_q, (q - 1) * errors_q)
            losses.append(tf.reduce_mean(loss_q))
        # 全クォンタイルの損失を平均
...
class TransformerPredictionRollingModel(BaseModel, PredictionModel):
    """
    将来の複数クォンタイル(価格分布)を予測する回帰モデル (CNN + Transformer + Quantile Regression)。
    """
    ...
    def _prepare_sequences(self, data):
        ...
        targets = np.array(targets).reshape(-1, 1)  # (サンプル数, 1)
        return sequences, targets

    # === ここを修正：最終Denseを「len(self.quantiles)」にする ===
    def create_cnn_transformer_model(
        ...
        # === ここがQuantile出力: len(self.quantiles) ===
        outputs = Dense(len(self.quantiles), activation=None, kernel_regularizer=l2(l2_reg))(x)
    ...
    def hyperparameter_search_optuna(
        ...
        def objective(trial):
            ...
            try:
                ...
                # === 3. コンパイル: lossをQuantile Lossに変更 ===
                q_loss = quantile_loss_factory(self.quantiles)
                model.compile(
                ...
                # 5. バリデーション損失 (quantile_loss の最終値)
        ...
        self.logger.log_system_message(f"Best trial loss: {study.best_value}")
    ...
    # --- ここで "直近サンプルほど重みが大きい" sample_weight を作成 ---
        sample_weights = self._create_sample_weights(len(x_train))
        ...
        self.model.fit(
            ...
            sample_weight=sample_weights  # ← 追加
        )

    """
    def _create_sample_weights(self, n: int, alpha: float = 0.99) -> np.ndarray:
        """
        学習サンプル数 n に対し、指数的に
        直近サンプルほど重みが大きくなる配列を返す。
        alpha は 0.95～0.995 など調整してみる。
        """
        # サンプルインデックス(0 ~ n-1)
        indices = np.arange(n)
        # 例: w[i] = alpha^(N-1-i)
        weights = alpha ** ((n - 1) - indices)
        return weights
    ...
    def train_with_cross_validation(self, data: np.ndarray, targets: np.ndarray) -> list:
        ...
        for train_idx, test_idx in kfold.split(data, targets):
            ...
            # 評価: val_lossを計算するには再度lossを計算するか、
            #       あるいはここではmeanを取るなどの方法もあり
            val_loss = model_cv.evaluate(data[test_idx], targets[test_idx], verbose=0)
    ...
    # === 評価をクォンタイルごとに算出する例に変更 ===
    def evaluate(self, x_test: np.ndarray, y_test: np.ndarray) -> dict:
        """
        出力 (N, len(self.quantiles)) に対し、y_test.shape = (N,1)。
        クォンタイルごとに MSE,MAE,R2 を計算し、リストとして返す。
        """
        y_pred_scaled = self.model.predict(x_test)  # shape: (N, len(self.quantiles))

        # 逆変換用に、クォンタイル方向を1次元ずつ処理
        y_pred_list = []
        for i in range(len(self.quantiles)):
            col_scaled = y_pred_scaled[:, i].reshape(-1, 1)
            col_inv = self.target_scaler.inverse_transform(col_scaled)
            y_pred_list.append(col_inv.reshape(-1))  # shape: (N,)

        # shape: (N,)
        y_true_inv = self.target_scaler.inverse_transform(y_test).reshape(-1)
        ...
        for i, q in enumerate(self.quantiles):
            mse_ = mean_squared_error(y_true_inv, y_pred_list[i])
            rmse_ = np.sqrt(mse_)
            mae_ = mean_absolute_error(y_true_inv, y_pred_list[i])
            r2_ = r2_score(y_true_inv, y_pred_list[i])

            metrics_result["MSE"].append(mse_)
            metrics_result["RMSE"].append(rmse_)
            metrics_result["MAE"].append(mae_)
            metrics_result["R2"].append(r2_)
    ...
    def plot_predictions(self, x_test: np.ndarray, y_test: np.ndarray, sample_size=300):
        """
        sample_sizeぶんの予測結果を表示。
        各クォンタイルの予測値を折れ線で描画し、実測値も重ねる。
        """
        y_pred_scaled = self.model.predict(x_test)  # (N, len(self.quantiles))
        N = len(y_test)
        if sample_size > N:
            sample_size = N

        # 後ろから sample_size 個だけ抜き出す
        y_true_inv = self.target_scaler.inverse_transform(y_test).reshape(-1)
        y_true_plot = y_true_inv[-sample_size:]

        # クォンタイルごとの予測を逆変換して取り出す
        y_pred_plot_list = []
        for i in range(len(self.quantiles)):
            col_scaled = y_pred_scaled[:, i].reshape(-1, 1)
            col_inv = self.target_scaler.inverse_transform(col_scaled).reshape(-1)
            y_pred_plot_list.append(col_inv[-sample_size:])

        # プロット
        plt.figure(figsize=(10, 6))
        # 実測
        plt.plot(range(sample_size), y_true_plot, label="Actual Price", marker='o')

        # クォンタイル予測
        for i, q in enumerate(self.quantiles):
            plt.plot(range(sample_size), y_pred_plot_list[i],
                     label=f"Predicted Q{int(q*100)}", linestyle='--')

        plt.title("BTC Price (Quantile Prediction) vs Actual")
    ...
    # === 予測は (N, len(self.quantiles)) を返す ===
    def predict(self, data: np.ndarray, use_gpu: bool = None) -> np.ndarray:
        """
        data.shape: (N, time_period, feature_dim)
        戻り値: (N, len(self.quantiles)) （スケールを逆変換済み）
        """
        if use_gpu is not None:
            original_devices = tf.config.get_visible_devices()
            self._configure_gpu(use_gpu)

        y_pred_scaled = self.model.predict(data)  # (N, len(self.quantiles))

        if use_gpu is not None:
            tf.config.set_visible_devices(original_devices)

        # 各クォンタイルを逆変換し、最終的に (N, len(self.quantiles)) にまとめる
        preds_inv = []
        for i in range(len(self.quantiles)):
            col_scaled = y_pred_scaled[:, i].reshape(-1, 1)
            col_inv = self.target_scaler.inverse_transform(col_scaled)
            preds_inv.append(col_inv)

        return np.hstack(preds_inv)

    # === 1点予測を返す場合は、代表値(Q50など)を返すか、あるいは複数返すかを選ぶ。例では Q50 を返す。===
    def predict_single(self, data_point: np.ndarray, use_gpu: bool = None) -> float:
        """
        1点だけの入力に対してQ50(中央値)の予測を返す例。
        """
        ...
        y_pred_scaled = self.model.predict(reshaped_data)  # (1, len(self.quantiles))

        if use_gpu is not None:
            tf.config.set_visible_devices(original_devices)

        # Q50のインデックスを量的に探す (上で [0.1,0.5,0.9] としているので index=1)
        q50_idx = self.quantiles.index(0.5)
        q50_val_scaled = y_pred_scaled[0, q50_idx].reshape(1, 1)
        q50_val_inv = self.target_scaler.inverse_transform(q50_val_scaled)

        return float(q50_val_inv[0][0])
...
def main():
    ...
    print("Test metrics (each quantile):", metrics)
    ...
    metrics2 = model.evaluate(x_test2, y_test2)
    print("Final evaluation on recent data (each quantile):", metrics2)
    model.plot_predictions(x_test2, y_test2, sample_size=50)
