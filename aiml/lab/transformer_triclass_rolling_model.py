...
class TransformerPredictionRollingModel(BaseModel, PredictionModel):
    ...
    def predict_single_res(self, data_point: np.ndarray, use_gpu: bool = None) -> np.ndarray:
        """
        Makes a prediction for a single data point and returns the raw result (3次元確率).
        """
    ...
    def compute_gradcam(self, data, layer_name="transformer_block_4", pred_index=None):
        """
        Computes Grad-CAM heatmap for the given data. (Optional)
        """
...
def main():
    ...
    model.set_parameters(param_epochs=60, n_splits=2)
