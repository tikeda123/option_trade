...
from aiml.garch_price_distribution_model import GarchPriceDistributionModel
...
def garch_price_distribution_analysis(current_date: str, price_col="close",symbol:str="BTCUSDT",day_of_interval:int=3):
    """
    GARCHモデルを用いて価格の分布を分析する。
    """
    data_loader = MongoDataLoader()
    #day_of_interval_min = day_of_interval * 1440
    day_of_interval_min = 720
    df = data_loader.load_data_from_datetime_period(
        start_date="2020-01-01 00:00:00",
        end_date=current_date,
        coll_type=MARKET_DATA_TECH,
        symbol="BTCUSDT",
        interval=day_of_interval_min
    )
    last_date = df["date"].iloc[-1]
    model = GarchPriceDistributionModel()
    returns = model.prepare_data(df, price_col=price_col)
...
def prediction_option_model(pred_type:int,current_date:str,current_price:float=None,symbol="BTCUSDT",day_of_interval=3):
    """
    ...
    result = garch_price_distribution_analysis(current_date, price_col="close",symbol="BTCUSDT",day_of_interval=day_of_interval)
    if current_price is None:
        current_price = result["forecast_price_quantiles"][0.5]


    entry_price, exit_price = decide_entry_exit(
        current_price,
        q10_price=result["forecast_price_quantiles"][0.1],
        q50_price=result["forecast_price_quantiles"][0.5],
        q90_price=result["forecast_price_quantiles"][0.9],
        r_q10=result["forecast_returns_quantiles"][0.1],
        r_q50=result["forecast_returns_quantiles"][0.5],
        r_q90=result["forecast_returns_quantiles"][0.9],
