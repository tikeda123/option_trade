...
class BybitTrader(BybitBaseAPI):
        ...
        def init_managers(self) -> None:
                ...
                try:
                        self._current_leverage = self.position_manager.get_my_leverage()
                        if self._current_leverage != INITIAL_LEVERAGE:
                            self.position_manager.set_my_leverage(INITIAL_LEVERAGE)
        ...
        def trade_entry_trigger(
                ...
                if target_price is None:
                        ...
                        target_price = self.price_round(current_price )
                        #trigger_price = self.adjust_trigger_price(trade_type, trigger_price, current_price)

                if trigger_price is not None:
                        trigger_price = self.price_round(trigger_price)
                ...
                try:
                        self.order_id = self.order_manager.trade_entry_trigger(
                                        qty, trade_type, target_price, trigger_price
                                )

                        if stop_loss_price is not None:
                                stop_loss_price = self.price_round(stop_loss_price)
                                self.order_manager.set_trading_stop(stop_loss=stop_loss_price,tpsl_mode="Full",position_idx=0)
...
def main():
    ...
    bybit_trader.position_manager.set_my_leverage(leverage)
'''
def main():
        bybit_trader = BybitTrader()
        ...
        current_leverage = bybit_trader.position_manager.get_my_leverage()
        print(f"Current leverage: {current_leverage}")

        exit(0)

        test_long_flag = False# Place a long entry trigger order

        if test_long_flag:
                target_price  = current_price * 0.999
                trigger_price = current_price * 0.999
                stop_loss_price = current_price * 0.98
                entry_tpye = ENTRY_TYPE_LONG
        else:
                target_price  = current_price * 1.001
                trigger_price = current_price * 1.001
                stop_loss_price = current_price * 1.02
                entry_tpye = ENTRY_TYPE_SHORT

        #order_id = bybit_trader.trade_entry_trigger(
        #        qty=0.1, trade_type=ENTRY_TYPE_LONG, target_price=target_price, trigger_price=trigger_price, stop_loss_price=stop_loss_price)

        bybit_trader.position_manager.set_my_leverage(5)
        exit(0)

        order_id = bybit_trader.trade_entry_trigger(qty=0.0005, trade_type=entry_tpye, target_price=target_price,stop_loss_price=stop_loss_price)
        ...
        #bybit_trader.order_manager.set_trading_stop(stop_loss=stop_loss_price,tpsl_mode="Full",position_idx=0)
        ...
        order_id = bybit_trader.trade_exit(qty=0.001, trade_type=entry_tpye)
