...
def generate_signal_generic(
    ...
    elif method == 'next_change':
        signal = np.where(series < series.shift(-1), 1, -1)
...
def main():
    ...
    # 'start_at', 'close', 'macdhist', 'rsi', 'roc' などがある想定
    # （無い場合はコメントアウト）
    df = df[['start_at', 'close', 'macdhist', 'rsi', 'roc','close_kalman','kalman_macdhist']].copy()
    ...
    def strategy_macdhist_increase(df):
        return generate_signal_generic(
            ...
            method='next_change'   # 前日比
        )

    def strategy_kalman(df):
        return generate_signal_generic(
            df,
            indicator_col='kalman_macdhist',
            method='next_change'   # 前日比
        )
