...
def simulate_option_prices(start_date: datetime, end_date: datetime, symbol) -> pd.DataFrame:
    ...
    #df_ivを時系列に並び替え
    df_iv = df_iv.sort_values('start_at')
    print(df_iv)
    #print(df_iv)
...
def main():
    ...
    plt.plot(pd.to_datetime(df_sim_op['timestamp']), df_sim_op['hv'], label='Simulated hv', marker='o')
    plt.plot(df_real_op.index, df_real_op['bid1Iv'], label='Real Bid IV', marker='o')
    plt.plot(df_real_op.index, df_real_op['ask1Iv'], label='Real Ask IV', marker='o')
    plt.title(f'Option Price Comparison for {symbol}')
    plt.xlabel('Date')
    plt.ylabel('IV')
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig('option_iv_comparison.png')
    plt.show()
    plt.close()
