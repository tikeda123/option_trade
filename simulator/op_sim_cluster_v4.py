...
def main():
    ...
    # === 【Pair Plot】===
    # 以下を追加することで、avg_rsi, avg_macdhist, avg_volume, avg_volatility, pct_return を含む
    # ペアプロットを表示し、クラスタラベルで色分けします
    plt.figure(figsize=(10, 8))
    sns.pairplot(
        data=feature_df,
        vars=['avg_rsi', 'avg_macdhist', 'avg_volume', 'avg_volatility', 'pct_return'],  # ここにpct_returnを追加
        hue='cluster_label',
        diag_kind='kde',
        palette=['blue', 'red', 'green', 'orange']
    )
    plt.suptitle("Pair Plot of Selected Features (incl. pct_return) by Cluster", y=1.02)
    plt.show()
