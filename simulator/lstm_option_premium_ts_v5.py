...
def main():
    ...
    for train_index, test_index in tscv.split(X_all):
        ...
        # ---- LSTMモデルを毎回新規構築 or 再学習 ----
        #   実用途に合わせ、Fold ごとに別モデルを作るか、あるいは最終Foldだけで評価するかは要検討

        # すでにモデルが保存されているならロード、なければ作成
        # ※ 交差検証でモデルを都度上書きすると最終だけしか残らない点に注意
    ...
    # ---------------------------------------
    # 以下、可視化などはあくまで参考例
    # (交差検証では fold ごとに test が異なるので、可視化は要工夫)
    # ---------------------------------------
