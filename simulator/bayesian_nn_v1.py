...
def main():
    ...
    def negative_log_likelihood_and_kl(output, target, kl_weight=0.01):
        ...
        kl_weightを少し小さめ(0.01など)にしてサンプリングの変動を抑制
        """
        mu      = output[:, 0]
        log_var = output[:, 1]
    ...
    with torch.no_grad():
        for batch_X, batch_y in test_loader:
            ...
            log_var_mean = torch.clamp(log_var_mean, min=-10, max=10)
    ...
    # -------------------------------------------------
    # 学習済みモデルを使って「ある時刻までのデータから予測する」関数を呼ぶ例
    # -------------------------------------------------
    target_time = datetime(2025, 2, 15, 0, 0)  # 例えば 2025-02-15 12:00 の直後を予測
    pred_mean, pred_std = predict_price_and_distribution(
        model,
        df=graph_df,
    ...
    print(f"\n--- 単発予測結果 ({target_time} の直後) ---")
    print(f"予測平均: {pred_mean:.2f}, 標準偏差: {pred_std:.2f}")


def predict_price_and_distribution(
    model: nn.Module,
    df: pd.DataFrame,
    ...
    """
    指定した target_time までのデータを用いて、その直後の価格を予測する。
    - model: 学習済みモデル (LSTMWithBayesianOutput)
    - df: 'start_at' カラムを含む DataFrame（学習時と同じ構造）
    - target_time: 予測したい「時刻（の直後）」を datetime で指定
    - seq_length: 過去何ステップを1系列とするか（学習時と同じ値）
    - scaler_X, scaler_y: 学習時に使用した StandardScaler
    - n_samples: 複数サンプリングする回数

    戻り値: (予測平均, 予測標準偏差) [共に元スケール]
    """
    model.eval()

    # target_time 以下のデータを抽出
    df_sorted = df.sort_values('start_at').reset_index(drop=True)
    # 時刻が target_time 以下の行を取得
    df_sub = df_sorted[df_sorted['start_at'] <= target_time].copy()
    if len(df_sub) < seq_length:
        raise ValueError("指定時刻までのデータが seq_length に満たないため、予測できません。")

    # seq_length 分だけ取り出す (最後の seq_length 行)
    df_input = df_sub.iloc[-seq_length:].copy()

    # 入力特徴量カラムは学習時と同じ
    lag_feature_cols = [ 'close', 'volume', 'macdhist', 'rsi', 'volatility', 'mfi']
    X_input_raw = df_input[lag_feature_cols].values  # shape: (seq_length, num_features)

    # スケーリング
    X_input_scaled = scaler_X.transform(X_input_raw)
    # PyTorch tensor 化 (batch_size=1)
    X_input_tensor = torch.tensor(X_input_scaled, dtype=torch.float32).unsqueeze(0)
    # shape => (1, seq_length, num_features)

    # ------ 複数サンプリングする例 ------
    with torch.no_grad():
        mu_avg, var_avg = multiple_sampling_predict(model, X_input_tensor, n_samples=n_samples)

    # mu_avg, var_avg は shape: (1,) => 1要素
    mu_avg = mu_avg.item()
    var_avg = var_avg.item()

    # 標準化解除
    y_scale = scaler_y.scale_[0]
    y_mean  = scaler_y.mean_[0]

    pred_mean_org = mu_avg * y_scale + y_mean
    pred_std_org  = np.sqrt(var_avg + 1e-8) * y_scale

    return pred_mean_org, pred_std_org
