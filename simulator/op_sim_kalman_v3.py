...
class ExtendedKalmanFilterTrendDetector:
    ...
    def visualize(self, results, title="Bitcoin/USDT Trend Analysis with Extended Kalman Filter"):
        ...
        ax1.plot(results.index, results['filtered_level'] * results['price'].mean(),
                 label='Filtered Level', color='red', linewidth=2)
        ...
        for i in range(len(results)):
            ...
            if prev_state is not None and (current_state != prev_state or i == len(results) - 1):
                ...
                alpha = 0.2 if prev_state in ['Uptrend', 'Downtrend'] else 0.1
        ...
        ax1.set_title('Price Chart and Extended Kalman Filter Estimated Level')
