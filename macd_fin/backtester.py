...
def check_doten(
    ...
    """
    ドテン時に 2σ 条件も考慮する。
    - 現在 LONG で、前日シグナルが負 => クローズ + (価格が lower2 を下回っていればショート)
    - 現在 SHORT で、前日シグナルが正 => クローズ + (価格が upper2 を上回っていればロング)
    """
    side = position_state["side"]

    # 例: LONG保有中 & シグナル反転(負) => ドテン
    if side == LONG and signal_yesterday < 0:
        ...
        # ---- ここで、2σのチェックを行った上で新たにSHORTエントリーするか判断 ----
        #     「シグナル<0 かつ current_price < lower2_value」のときショート
        if (current_price < lower2_value) and (signal_yesterday < 0):
    ...
    # 例: SHORT保有中 & シグナル反転(正) => ドテン
        ...
        # ---- ここで、2σのチェックを行った上で新たにLONGエントリーするか判断 ----
        #     「シグナル>0 かつ current_price > upper2_value」のときロング
        if (current_price > upper2_value) and (signal_yesterday > 0):
